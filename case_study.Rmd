...
Perform the Kolmogorov-Smirnov test across distributions:
...
Check Missing Values and Discuss Imputation Technique

```{r}
# Get the count of NaN (NA) values for each column
na_counts <- sapply(dta, function(x) sum(is.na(x)))

# Display the NA counts for each column
cat("Missing values in 'dta':\n")
print(na_counts)
cat("\n")

# Get the count of NaN (NA) values for each column
na_counts <- sapply(scoring, function(x) sum(is.na(x)))

# Display the NA counts for each column
cat("Missing values in 'scoring':\n")
print(na_counts)
```
...
```{r}
# Create a renamed copy of the data without the "outcomes" field
dta_renamed <- dta %>% select(-outcome)

# Combine dta_renamed and scoring vertically (row-wise)
combined_part_2_data <- bind_rows(dta_renamed, scoring)
...
# View the resulting dataset with 2 principal components
head(pca_data)
```
